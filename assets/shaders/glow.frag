...
void main() {
    ...
    //    for(int i = 1; i < 5; ++i)
    //    {
    //        result += texture(diffuseImage, texCoords + vec2(tex_offset.x * i, 0.0)).rgb * weight[i];
    //        result += texture(diffuseImage, texCoords - vec2(tex_offset.x * i, 0.0)).rgb * weight[i];
    //    }
	//	blurOutput = result;
    //} else {
    //    for(int i = 1; i < 5; ++i)
    //    {
    //        result += texture(prevBlurImage, texCoords + vec2(0.0, tex_offset.y * i)).rgb * weight[i];
    //        result += texture(prevBlurImage, texCoords - vec2(0.0, tex_offset.y * i)).rgb * weight[i];
    //    }
	//	finalOutput = result;
    //}
	blurOutput = vec3(1,0,0);
