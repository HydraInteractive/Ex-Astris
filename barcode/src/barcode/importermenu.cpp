...
void ImporterMenu::render(bool &closeBool, Hydra::Renderer::Batch* previewBatch, float delta)
{
	...
	if (ImGui::Begin("Import", &closeBool, ImGuiWindowFlags_MenuBar)) {
		_menuBar();
	...
	if (ImGui::TreeNodeEx(this, nodeFlags, label, _name.c_str())) {
		...
		for (size_t i = 0; i < this->_files.size(); i++) {
			...
			if (ext == ".mattic" || ext == ".mATTIC") {
				if (ImGui::TreeNodeEx(_files[i], nodeFlags | ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen, ICON_FA_CUBE " %s", _files[i]->_name.c_str())) {
					if (ImGui::IsItemClicked()) {
				...
				if (ImGui::TreeNodeEx(_files[i], nodeFlags | ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen, ICON_FA_CUBES " %s", _files[i]->_name.c_str())) {
					if (ImGui::IsItemClicked()) {
						(*selectedNode) = _files[i];
				...
				ImGui::TreeNodeEx(_files[i], nodeFlags | ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen, ICON_FA_QUESTION_CIRCLE_O " %s", _files[i]->_name.c_str());
		}
		ImGui::TreePop();
	}
}
