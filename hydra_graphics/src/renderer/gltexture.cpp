...
public:
	...
	void bindToComputeShader(size_t position, bool read) {
		glActiveTexture(static_cast<GLenum>(GL_TEXTURE0 + position));
		if(read)
			glBindImageTexture((GLuint)position, _texture, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA8);
		else
			glBindImageTexture((GLuint)position, _texture, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA8);
	}
