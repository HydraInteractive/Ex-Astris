...
namespace Hydra::Component {
	struct HYDRA_GRAPHICS_API LightComponent final : public IComponent<LightComponent, ComponentBits::Light> {
		...
		inline glm::mat4 getViewMatrix() { return glm::lookAt(getTransformComponent()->position, getTransformComponent()->position + glm::vec3(0, -1, 0), glm::vec3(0, 0, 1)); }
		...
		inline glm::vec3 getDirVec(){
			return glm::normalize(getTransformComponent()->position + glm::vec3(0, -1, 0));
		}
	};
}
